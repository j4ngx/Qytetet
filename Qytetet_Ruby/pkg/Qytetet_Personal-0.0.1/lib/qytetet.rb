...
class Qytetet
  ...
  def actuar_si_en_casilla_edificable
    if @jugador_actual.debo_pagar_alquiler
  ...
  def actuar_si_en_casilla_no_edificable
    if @jugador_actual.get_casilla_actual.get_tipo_casilla==TipoCasilla::IMPUESTO
  ...
  def aplicar_sorpresa
    ...
    if @carta_actual.get_tipo==TipoSorpresa::IRACASILLA
      if(@carta_actual.get_valor==@tablero.get_carcel.get_numero_casilla)
        encarcelar_jugador
      else
        mover(@carta_actual.get_valor)
      end
    end
    if @carta_actual.get_tipo==TipoSorpresa::PAGARCOBRAR
      @jugador_actual.modificar_saldo(@carta_actual.get_valor)
    end
    if @carta_actual.get_tipo==TipoSorpresa::PORJUGADOR
      @jugador_actual.modificar_saldo(@carta_actual.get_valor*@num_jugadores)
      for i in 0..@jugadores.size()-1
        if(@jugador_actual!=@jugadores[i])
          @jugadores[i].modificar_saldo(@carta_actual.get_valor)
        end
      end
    end
    if @carta_actual.get_tipo== TipoSorpresa::PORCASAHOTEL
      casa_hoteles=0
      for i in 0..@jugador_actual.get_propiedades.size()-1
        casa_hoteles += @jugador_actual.get_propiedades[i].get_numcasas
        casa_hoteles += @jugador_actual.get_propiedades[i].get_numhoteles
      end
      @jugador_actual.modificar_saldo(@carta_actual.get_valor * casa_hoteles)
    end
  end
  ...
  def inicializar_cartas_sorpresa
        ...
        @mazo<<Sorpresa.new("Te han pillado robando comida en el Lecho de "\
            "Pulgas ,debes ir a las mazmorras de Desembarco del Rey.", TipoSorpresa::IRACASILLA,@tablero.get_carcel.get_numero_casilla)
        ...
        @mazo<<Sorpresa.new("Los saqueos han comenzado en las ciudades por escasez de" \
                 ...
                 "repararlos, pagas 100 monedas de oro por posada y " \
        ...
        @mazo<<Sorpresa.new("La gran gestion de La Mano(Tyrion Lannister) hace " \
                 ...
                 "recuperar el honor de otras epocas, recibes 100 monedas de oro por posada y " \
                 "por castillo.",TipoSorpresa::PORCASAHOTEL,100)
        ...
        @mazo<<Sorpresa.new("Reunes a los jugadores de la grandes casas para tratar de " \
                ...
                "250 monedas de oro en compensacion",TipoSorpresa::PORJUGADOR,250)
        ...
        @mazo<<Sorpresa.new("La guerra con los demas jugadores por tu intento de separar " \
                ...
                "250 monedas de oro a cada jugadores",TipoSorpresa::PORJUGADOR,-250)
  end
